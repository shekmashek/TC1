...
import { initialRates } from './utils/initialRates';
...
import VatTablePage from './pages/VatTablePage';
...
function AppContent() {
  ...
  useEffect(() => {
    const completed = localStorage.getItem('onboardingCompletedProjects');
    if (completed) {
        try {
            const parsed = JSON.parse(completed);
            if (Array.isArray(parsed)) {
                uiDispatch({ type: 'SET_ONBOARDING_COMPLETED_PROJECTS', payload: parsed });
            }
        } catch (e) {
            console.error("Failed to parse onboarding status from localStorage", e);
        }
    }
  }, [uiDispatch]);

  useEffect(() => {
    dataDispatch({ type: 'SET_EXCHANGE_RATES', payload: initialRates });
    const lastFetch = localStorage.getItem('exchangeRatesLastFetch');
    const now = new Date();
    const oneDay = 24 * 60 * 60 * 1000;
    if (!lastFetch || (now - new Date(lastFetch)) > oneDay) {
        supabase.functions.invoke('get-exchange-rates')
            .then(({ data, error }) => {
                if (error) {
                    console.warn("Could not fetch live exchange rates. This is expected in some dev environments. Using fallback rates.");
                    return;
                }
                if (data.result === 'success') {
                    dataDispatch({ type: 'SET_EXCHANGE_RATES', payload: data.conversion_rates });
                    localStorage.setItem('exchangeRatesLastFetch', new Date().toISOString());
                } else {
                    console.error("Failed to update exchange rates from API:", data.error || 'Unknown API error');
                }
            })
            .catch(() => {
    ...
    if (session && !profile) {
      const fetchInitialData = async () => {
        ...
        try {
          ...
          if (!profileData) {
            throw new Error("La création ou le chargement de votre profil a échoué. Veuillez réessayer ou contacter le support.");
          }
          ...
          const { data: acceptedCollaborations, error: collabError } = await supabase
            .from('collaborators')
            .select('project_ids')
            .eq('user_id', user.id)
            .eq('status', 'accepted');
          ...
          const collaboratedProjectIds = (acceptedCollaborations || []).flatMap(c => c.project_ids).filter(Boolean);
          
          const ownedProjectsFilter = `user_id.eq.${user.id}`;
          const collaboratedProjectsFilter = collaboratedProjectIds.length > 0 ? `id.in.(${collaboratedProjectIds.join(',')})` : null;
          let projectsFilter = ownedProjectsFilter;
          if (collaboratedProjectsFilter) {
              projectsFilter = `or(${ownedProjectsFilter},${collaboratedProjectsFilter})`;
          }

          const { data: projectsData, error: projectsError } = await supabase.from('projects').select('*').or(projectsFilter);
          if (projectsError) throw projectsError;

          if (projectsData) {
            const icons = ['Home', 'Briefcase', 'Plane', 'Heart', 'Coffee', 'HeartHandshake', 'Book', 'PiggyBank', 'Car', 'Building2', 'ShoppingBasket', 'Gift'];
            const colors = ['red', 'orange', 'amber', 'yellow', 'lime', 'green', 'emerald', 'teal', 'cyan', 'sky', 'blue', 'indigo', 'violet', 'purple', 'fuchsia', 'pink', 'rose'];
            
            const projectsToUpdate = [];
            projectsData.forEach((p, index) => {
                if (!p.icon || !p.color) {
                    const newIcon = p.icon || icons[index % icons.length];
                    const newColor = p.color || colors[index % colors.length];
                    projectsToUpdate.push({ id: p.id, icon: newIcon, color: newColor });
                    p.icon = newIcon;
                    p.color = newColor;
                }
            });
            ...
            if (projectsToUpdate.length > 0) {
                supabase.from('projects').upsert(projectsToUpdate).then(({ error }) => {
                    if (error) {
                        console.warn("Could not auto-assign icons/colors to projects:", error);
                    }
                });
            }
          }
          ...
          const [
            tiersRes, loansRes, scenariosRes, 
            ...
            vatRatesRes, vatRegimesRes, taxConfigsRes, personnelChargesRes,
          ] = await Promise.all([
            supabase.from('tiers').select('*'), supabase.from('loans').select('*'),
            ...
            supabase.from('templates').select('*'), supabase.from('user_categories').select('*'),
            supabase.from('vat_rates').select('*'), supabase.from('vat_regimes').select('*'), supabase.from('tax_configs').select('*'),
            supabase.from('personnel_charges').select('*'),
          ]);

          const responses = { projectsRes: { error: projectsError, data: projectsData }, tiersRes, loansRes, scenariosRes, entriesRes, actualsRes, paymentsRes, cashAccountsRes, scenarioEntriesRes, consolidatedViewsRes, collaboratorsRes, commentsRes, templatesRes, customCategoriesRes, vatRatesRes, vatRegimesRes, taxConfigsRes, personnelChargesRes };
          ...
          userIds.add(user.id);
          if (projectsData) { projectsData.forEach(p => userIds.add(p.user_id)); }
          ...
          const projectsForState = (projectsData || []).map(p => ({ 
              id: p.id, name: p.name, description: p.description, currency: p.currency, startDate: p.start_date, endDate: p.end_date, isArchived: p.is_archived, user_id: p.user_id, annualGoals: p.annual_goals, expenseTargets: p.expense_targets, display_unit: p.display_unit, decimal_places: p.decimal_places, type: p.type, onboarding_step: p.onboarding_step, icon: p.icon, color: p.color, dashboard_widgets: p.dashboard_widgets,
          ...
          const collaborators = (collaboratorsRes.data || []).map(c => ({ id: c.id, ownerId: c.owner_id, userId: c.user_id, email: c.email, status: c.status, projectIds: c.project_ids, permissions: c.permissions }));
          ...
          const personnelCharges = personnelChargesRes.data || [];

          const allEntries = (entriesRes.data || []).reduce((acc, entry) => {
            const e = { id: entry.id, loanId: entry.loan_id, type: entry.type, category: entry.category, frequency: entry.frequency, amount: entry.amount, date: entry.date, startDate: entry.start_date, endDate: entry.end_date, supplier: entry.supplier, description: entry.description, isOffBudget: entry.is_off_budget, payments: entry.payments, provisionDetails: entry.provision_details, isProvision: entry.is_provision, currency: entry.currency, original_amount: entry.original_amount, amount_type: entry.amount_type, vat_rate_id: entry.vat_rate_id, ht_amount: entry.ht_amount, ttc_amount: entry.ttc_amount, userId: entry.user_id };
          ...
          const allCashAccounts = (cashAccountsRes.data || []).reduce((acc, account) => {
            const a = { id: account.id, projectId: account.project_id, mainCategoryId: account.main_category_id, name: account.name, initialBalance: account.initial_balance, initialBalanceDate: account.initial_balance_date, isClosed: account.is_closed, closureDate: account.closure_date, currency: account.currency };
          ...
          const taxConfigs = taxConfigsRes.data || [];

          dataDispatch({
            type: 'SET_INITIAL_DATA',
            payload: {
              allProfiles: profilesData || [], settings, projects: projectsForState, tiers, loans, scenarios, consolidatedViews, collaborators, allComments, templates, vatRates, vatRegimes, taxConfigs, personnelCharges,
          ...
          if (isNewSignup && projectsData.length === 0) {
            const defaultProjectPayload = {
                projectName: 'Mon Premier Projet',
                projectDescription: 'Un projet pour commencer à explorer Trezocash',
            ...
            await initializeProject({ dataDispatch, uiDispatch }, defaultProjectPayload, session.user, tiers, templates);
            localStorage.removeItem('isNewSignup');
          } else if (projectsData.length === 0) {
            uiDispatch({ type: 'START_ONBOARDING', payload: { projectClass: 'treasury' } });
          } else {
            const currentActiveId = uiState.activeProjectId;
            const allProjectIds = projectsForState.map(p => p.id);
            if (currentActiveId && allProjectIds.includes(currentActiveId)) {
                uiDispatch({ type: 'SET_ACTIVE_PROJECT', payload: currentActiveId });
            } else {
  ...
  }, [session, profile, dataDispatch, uiDispatch]);
  ...
  const ProtectedRoute = () => {
    ...
    if (!location.pathname.startsWith('/admin') && (uiState.isOnboarding || (projects.length === 0 && localStorage.getItem('isNewSignup') !== 'true'))) {
      ...
      <Routes>
          <Route element={<ProtectedRoute />}>
              <Route path="/admin" element={<AdminLayout />}>
                  ...
                  <Route path="referrals" element={<AdminReferralManagementPage />} />
              </Route>
              <Route path="/app" element={<AppLayout />}>
                <Route index element={<Navigate to="projets" replace />} />
                <Route path="projets" element={<MyProjectsPage />} />
                ...
                <Route path="emprunts-prets" element={<LoansPage />} />
                <Route path="tva" element={<VatManagementPage />} />
                <Route path="tableau-tva" element={<VatTablePage />} />
                <Route path="fiscalite" element={<TaxManagementPage />} />
                <Route path="fiscalite/:taxId" element={<TaxSettingsPage />} />
                <Route path="parametres-projet" element={<ProjectSettingsPage />} />
                <Route path="delete-account" element={<DeleteAccountPage />} />
                <Route path="en-retard" element={<OverduePage />} />
